...
export function WebsiteInfoModal({ websiteUrl }: { websiteUrl: string }) {
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <Button size="sm" variant="outline"><Code className="mr-2 size-4 stroke-2" />Add to Your Website</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <SmileGhost className="mb-2 size-8 shrink-0 text-orange-500 dark:text-orange-300" />
          <DialogTitle className="text-left">Let&apos;s add to your website!</DialogTitle>
        ...
        <DialogDescription className="space-y-4 text-sm">
          <Tabs defaultValue="home" className="w-full">
            <TabsList className="w-full">
              <TabsTrigger className="w-full" value="home">Home Page</TabsTrigger>
              <TabsTrigger className="w-full" value="subpages">Sub Pages</TabsTrigger>
            </TabsList>
          ...
          <Tabs defaultValue="Next.js">
            <TabsList className="w-full">
              {Object.keys(tabContents).map(tab => (
                <TabsTrigger key={tab} className="w-full" value={tab}>{tab}</TabsTrigger>
              ))}
            </TabsList>
            {Object.entries(tabContents).map(([tab, content]) => (
              <TabsContent key={tab} value={tab}>
                <p className="mb-2">
                  {tab === 'Next.js' ? 'Add the meta tag to your page metadata export in Next.js:' :
                   tab === 'React' ? 'Use a library like react-helmet to add the meta tag:' :
          ...
          {['Learn more about Open Graph', 'Validate your metadata'].map((text, index) => (
            <a key={index} href={index === 0 ? 'https://ogp.me/' : 'https://metatags.io/'} target="_blank" rel="noopener noreferrer" className="flex items-center font-medium text-primary">
