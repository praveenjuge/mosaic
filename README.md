...
2. Enter `VITE_SITE_URL`, `VITE_CLERK_PUBLISHABLE_KEY`, and the secret `CLERK_SECRET_KEY` when prompted.
3. Deploy the Worker; the deploy command applies the D1 migration before publishing the application.

Cloudflare provisions fork-specific resources from `wrangler.jsonc`. To use a custom domain, add it in the Worker's **Settings → Domains & Routes**, set `VITE_SITE_URL` to that HTTPS origin in Workers Builds and Worker runtime variables, and allow the origin in Clerk.
