...
- **Screenshots**: Cloudflare Browser Run via the `env.BROWSER` binding, defaulting to Kitesurf (`fetch("…/screenshot?browser=kitesurf", …)`)
...
- OG image generation is handled internally via the Cloudflare Browser Run `BROWSER` Workers binding with `fetch("https://fake.host/screenshot?browser=kitesurf", …)` — Kitesurf by default (documented query-param opt-in), no account ID or API token required
