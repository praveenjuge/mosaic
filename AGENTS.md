...
- **Screenshots**: Cloudflare Browser Run via the `env.BROWSER` binding (`quickAction`), defaulting to Kitesurf (`browser: "kitesurf"`)
...
- OG image generation is handled internally via the Cloudflare Browser Run `BROWSER` Workers binding using `env.BROWSER.quickAction("screenshot", { browser: "kitesurf", ... })` — Kitesurf by default (binding has no query string; REST/CDP use `?browser=kitesurf`), no account ID or API token required
