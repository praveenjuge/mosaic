      ...
      <div className="fixed bottom-4 right-4 z-50">
        <Card className="w-full max-w-md shadow-lg">
          <CardHeader className="flex flex-row items-start justify-between space-y-0 p-4">
            <ClosePopover isOpen={isPopoverOpen} setIsOpen={setIsPopoverOpen} onClose={handleClose} />
            <CardTitles />
          ...
          <CardContent className='p-4 pt-2'>
            <ol className="space-y-4">
              {onboardingSteps.map((step, index) => (
                <OnboardingStep key={index} step={step} userData={userData} stepNumber={index + 1} />
              ))}
            </ol>
          </CardContent>
        </Card>
...
function ClosePopover({ isOpen, setIsOpen, onClose }: { isOpen: boolean, setIsOpen: (open: boolean) => void, onClose: () => void }) {
  return (
...
function MinimizeButton({ onClick }: { onClick: () => void }) {
  return (
    <Button variant="outline" size="sm" onClick={onClick} className="size-6 p-0">
...
function OnboardingStep({ step, userData, stepNumber }: { step: OnboardingStep, userData: UserMetaData, stepNumber: number }) {
  const isCompleted = step.isCompleted(userData)
  return (
    <li className={`${isCompleted ? 'pointer-events-none line-through opacity-40' : ''}`}>
      <Link href={step.href} className="flex items-start gap-3 rounded border-[0.5px] bg-primary-foreground p-3 transition-colors hover:bg-secondary">
        <div className={`flex size-6 shrink-0 items-center justify-center rounded-full border-[0.5px] text-xs font-medium ${isCompleted ? 'border-emerald-500 bg-emerald-500 text-primary-foreground' : 'bg-background'}`}>
